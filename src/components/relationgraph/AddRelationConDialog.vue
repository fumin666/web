...
*/
<template>
  <s-dialog v-if="dialogFlag" v-model="dialogFlag" width="800px" height="500px" title="关系列表" key="add-relation-dialog">
    <s-scrollbar wrap-style="max-height: 500px;">
      <s-row style="margin-bottom: 10px;">
        <s-col :span="14" style="height: 40px; line-height: 40px;">
          ...
          <s-select v-model="crelation" value-key="uuid" placeholder="请选择" :disabled="addWay !== '1'">
            <s-option v-for="item in userCoptions" :label="item.relationName + '--' + item.citypeName" :key="item.relationTypeUuid" :value="item">
            </s-option>
          </s-select>
        </s-col>
      ...
      <div v-if="addWay === '1'">
        <s-radio-group style="width: 100%;" v-model="currentRow" v-if="relationSelect.relationDefine === '1'">
          <s-table ref="singleTable" :data="configitemdataClone" highlight-current-row @current-change="handleCurrentChange" style="width: 100%" height="280">
        ...
        <s-table ref="multipleTable" :data="configitemdataClone" @selection-change="handleSelectionChange" style="width: 100%" height="280" v-else>
          <s-table-column type="selection" width="90">
          </s-table-column>
          <s-table-column prop="ciname" label="名称">
          </s-table-column>
          <s-table-column property="ciType" label="类型">
          </s-table-column>
          <s-table-column property="ip" label="IP地址">
        ...
        <div style="margin:0 5%;">
          <s-form ref="searchForm" :model="searchForm" :rules="serachFormRules" :inline="true">
            <s-form-item label="配置项三级分类" prop="selectedClassify">
              <select-tree v-model="searchForm.selectedClassify" url="/resourcemanager/configurationmanage/citype/getCitypeTree" name="selectedClassify" @selectTreeChange="handleSelectTreeChange">
              </select-tree>
            </s-form-item>
            <s-form-item label="" prop="value">
              <s-input v-model="searchForm.value" placeholder="请输入关键字搜索"></s-input>
            </s-form-item>
            <s-form-item>
          ...
          <s-form :model="this" :rules="typeRules" ref="userDefineCrelation">
            <s-form-item label="关系类型" label-width="100px" prop="userDefineCrelation">
              <s-select v-model="userDefineCrelation" value-key="uuid" placeholder="选择关系类型">
                <s-option v-for="item in userDefineCoptions" :label="item.forwardRelationtype.relationName" :key="item.relationTypeUuid" :value="item">
                </s-option>
              </s-select>
            </s-form-item>
          </s-form>
    ...
    <template slot="footer" class="dialog-footer">
      <s-button @click="submit(true)">继续添加</s-button>
      <s-button @click="submit">保存</s-button>
  ...
  methods: {
    getUserDefineCoptions() {
      $axios.get('/resourcemanager/configurationmanage/relationtype/list').then(({data}) => {
        let arr = [];
        data.map((item) => {
          if (item.forwardRelationtype.relationName === '连接') {
            if (item.forwardRelationtype.status === 1) {
              this.userCoptions = [...this.coptions]
            }
          } else {
            if (item.forwardRelationtype.status === 1) {
              arr.push(item);
    ...
    onSearch() {
      this.$refs.searchForm.validate((valid) => {
        if (valid) {
          let ciUuid = this.tipNode.id && this.isRoot ? this.tipNode.id : this.compId
          let {
            selectedClassify,
            value
          } = this.searchForm
    ...
    searchSubmit(flag) {
      ...
      this.$refs.userDefineCrelation.validate(valid => {
        if (valid) {
          ...
          $axios.post('/resourcemanager/configurationmanage/relationtype/savecirelation', data).then(({data}) => {
            if (data.state === true) {
              this.searchData = []
              this.$emit('submit')
      ...
      //        if (!this.userDefineCrelation.forwardUuid) {
      //          this.$message('请选择关系类型!')
      //          return
      //        }
    },
    addSubmit(flag) {
