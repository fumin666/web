  ...
  <section class="sys-upgrade">
    ...
    <s-row type="flex" justify="center">
      <s-upload
        ...
        <div class="s-upload__tip" slot="tip">内部版本：<span style="font-size: 16px">{{innerVersion}}</span>
        </div>
      </s-upload>
    </s-row>
      ...
      <s-row>
        <s-table-page :data='upgradeList'
          ...
          <s-table-column
            label="操作">
            <template slot-scope="scope">
              <i v-if="isShowUpgrade(scope.row)" @click="upgradeHandler(scope.row)" title="升级"
                 class="iconfont icon-update"></i>
