...
export default {
  data() {
    let checkRepeatName = (rule, value, callback) => {  // 校验重复邮件/短信类型
      let vm = this
      if (!vm.isEdit) {
        if (value) {
          $axios.get(`/systemmanager/sysNotice/checkSysNoticeName/${vm.platform}/${value}`).then((res) => {
    ...
    return {
      ...
      rules: {
        name: [Validaters.NotNull, {validator: checkRepeatName, trigger: 'blur'}]
      },
      typeName: ''
    }
  },
