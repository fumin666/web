  ...
  <section class="content logImportOrder">
    ...
    <s-form :inline="true" class="search_form" label-width="90px">
      <div>
        ...
        <s-form-item label="资产IP：">
          <s-input v-model="deviceIp"></s-input>
        </s-form-item>
      </div>
    ...
    <s-table-page
      ...
      >
      <s-table-column label="登录时间" prop="create_date"></s-table-column>
      <s-table-column label="登录IP" prop="session_client_ip"></s-table-column>
      <s-table-column label="登录名" prop="user_login_name"></s-table-column>
      <s-table-column label="访问资产" prop="device_ip"></s-table-column>
      <s-table-column label="访问协议" prop="protocol_name"></s-table-column>
      <s-table-column label="登录账号" prop="account_name"></s-table-column>
      <s-table-column label="使用命令" prop="cmdContent"></s-table-column>
    </s-table-page>
  </section>
</template>
...
//        this.collectTask.taskPeriod = Number(this.collectTask.taskPeriod);
      ...
      showList() {
        let vm = this;
        $axios.post('/keyCmdStat/keyCmdSessionList', {deviceIp: vm.deviceIp, endTime: vm.endTimes, startTime: vm.beginTimes, groupUuid: vm.groupUuid}).then((res) => {
      ...
      ExcelDownloadHandle() {
        let vm = this;
        $axios.post('/keyCmdStat/exportCmdByCmd', {exportType: 'excel', deviceIp: vm.deviceIp, endTime: vm.endTimes, startTime: vm.beginTimes, groupUuid: vm.groupUuid}).then((res) => {
          if (res.data === 'success') {
            Download(`/keyCmdStat/downCmdByCmd/excel?t=${new Date().getTime()}`);
          }
        });
      },
      PdfDownloadHandle() {
        let vm = this;
        $axios.post('/keyCmdStat/exportCmdByCmd', {exportType: 'pdf', deviceIp: vm.deviceIp, endTime: vm.endTimes, startTime: vm.beginTimes, groupUuid: vm.groupUuid}).then((res) => {
          if (res.data === 'success') {
            Download(`/keyCmdStat/downCmdByCmd/pdf?t=${new Date().getTime()}`);
          }
        });
      },
      WordDownloadHandle() {
        let vm = this;
        $axios.post('/keyCmdStat/exportCmdByCmd', {exportType: 'word', deviceIp: vm.deviceIp, endTime: vm.endTimes, startTime: vm.beginTimes, groupUuid: vm.groupUuid}).then((res) => {
          if (res.data === 'success') {
            Download(`/keyCmdStat/downCmdByCmd/word?t=${new Date().getTime()}`);
          }
        });
      }
    },
