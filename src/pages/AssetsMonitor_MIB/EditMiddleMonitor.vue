...
<script type="text/babel">
  ...
  export default{
    ...
    methods: {
      addMiddleMonitor() {
        this.$refs['middleMonitorForm'].validate((valid) => {
          if (valid) {
            if (this.editMiddleMonitorData.callerName === 'addMonitorType') {
              Bus.$emit('addMonitorType-closeMiddleMonitorDialog-edit', {$index: this.editMiddleMonitorData.$index, backData: this.middleMonitorForm})
            }
            if (this.editMiddleMonitorData.callerName === 'editMonitorType') {
              Bus.$emit('editMonitorType-closeMiddleMonitorDialog-edit', {$index: this.editMiddleMonitorData.$index, backData: this.middleMonitorForm})
            }
            Bus.$emit('closeMiddleMonitorDialog-edit')
          }
        })
      },
    ...
    created() {
      this.middleMonitorForm = Object.assign({}, this.editMiddleMonitorData.row)
    }
  }
</script>
