 ...
 * @Author: your name
 * @Date: 2019-09-03 18:11:14
 * @LastEditTime: 2019-09-17 17:49:10
 * @LastEditors: 高建鹏
 -->
<template>
  <div class="assetList">
        ...
        <div class="listChunk">
            ...
            <s-form :inline="true" :model="searchForm" class="search_form" label-width='80px'>
              <div>
                <s-form-item label="监控状态：">
                  <s-select v-model="searchForm.isMonitor">
                ...
                <s-form-item label="资产状态：">
                  <s-select v-model="searchForm.assetState" :disabled="pingStatusDisabled">
                    <s-option label="所有" value=""></s-option>
          ...
          <s-row class="searchBtChunk">
            <s-col class="left">
              ...
              <s-button v-if="!isConcern" @click="gotoScoreSet" :disabled="!ImpAuth">资产监控打分设置</s-button>
            </s-col>
            <s-col class="right">
          ...
          <RealPageTable v-if="isShow" :ajax-obj="ajaxObj" @selection-change="selectionChange" style="width:100%;" :callback="handleData">
            ...
            <s-table-column label="操作" width="100">
              <template slot-scope="scope">
                <i class="iconfont" :class="[scope.row.isConcern ? 'icon-attended-full' : 'icon-attended']"
...
<script>
  ...
  import treatyTestDialog from './MonitorDialog/TreatyTestDialog';
  ...
  export default {
    ...
    methods: {
      ...
      axiosMonitorTree(callback = function () {
        ...
        let url = '/monitorcitype/getConcernCountCitypeTree';
      ...
      axiosMonitorList(callback = () => {}) {
        ...
        if (this.isConcern) this.$set(param.condition, 'isConcern', 1);
      ...
      openMonitorBatch() {
        ...
        this.axiosBatchMonitor(param, this.selections, data => {
          if (data.state === true) {
      ...
      stopMonitorBatch() {
        ...
        let logParam = {
          ...
          logTemplate: '停止监控|资产(#ip#/#assetName#)'
        };
      ...
      addConcernBatch() {
        ...
        if (uuidArr.length === 0) {
          ...
          this.$confirm('确定关注？', '提示', {confirmButtonText: '确定', cancelButtonText: '取消', type: 'info'}).then(() => {
            $axios.post('/resourcemanager/assetmanager/itasset/batchAddConcern', uuidArr).then(({data}) => {
              if (data) {
      ...
      cacelConcernBatch() {
        ...
        if (uuidArr.length === 0) {
          ...
          this.$confirm('确定取消关注？', '提示', {confirmButtonText: '确定', cancelButtonText: '取消', type: 'info'}).then(() => {
            $axios.post('/resourcemanager/assetmanager/itasset/batchCancelConcern', uuidArr).then(({data}) => {
              if (data) {
                this.$message({showClose: true, message: data, type: 'success'});
                this.reload(true);
                this.selections = []  // 批量操作后清掉选中数目
      ...
      rowConcern(rowData) {
        $axios.post(`/resourcemanager/assetmanager/itasset/concern/${rowData.uuid}`).then(({data}) => {
          if (data) {
            this.$message({showClose: true, message: data, type: 'success'});
            this.reload(true);
          } else {
