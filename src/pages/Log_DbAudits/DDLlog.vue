  ...
  export default {
    ...
    methods: {
      ...
      getChart1() {
        // 数据库维度
        const form = this.formData()
        $axios.post('/dbauditReport/ddlDatabaseTimeStat', form).then((res) => {
          let arrdata = JSON.parse(JSON.stringify(res.data.data));
          let arrrelation = JSON.parse(JSON.stringify(res.data.relation));
          let relation = {};
          for (let i in arrrelation) {
      ...
      getList() {
        const form = this.formData()
        $axios.post('/dbauditReport/ddlDataCmdTimeStat', form).then((res) => {
      ...
      downloadHandle(type) {
        let form = this.formData()
          Download(`/dbauditReport/ddlDatabaseTimeStatExport/${form.startTime}/${form.endTime}/${form.timeType}/${type}`);
      }
    },
    mounted() {
      this.getList();
