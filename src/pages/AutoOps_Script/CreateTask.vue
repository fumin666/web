        ...
        <s-collapse-item title="基础设定" name="base">
          <s-form :model="taskForm" :rules="rules1" ref="taskForm" label-width="120px">
            ...
            <s-form-item label="优先级" prop="priority" style="width: 100%">
              <s-select v-model="taskForm.priority" style="width: 100%">
            ...
            <s-form-item label="是否周期任务" prop="isPeriodicTask">
              <s-radio-group v-model="taskForm.isPeriodicTask">
                <s-radio :label="0">否</s-radio>
                <s-radio :label="1">是</s-radio>
              </s-radio-group>
            </s-form-item>
            <div v-if="current">
              <s-period v-if="timeFlag" v-model="taskForm.tScheduleTime" :isShowNow="'is-show-now'"　:showPeriodItem="showPeriodItem" ref="period"></s-period>
            </div>
            <div v-else>
              <s-period v-model="taskForm.tScheduleTime" :isShowNow="'is-show-now'"　:showPeriodItem="showPeriodItem" ref="period"></s-period>
            </div>
            <s-form-item label="备注">
              <s-input type="textarea" v-model="taskForm.remark"></s-input>
            </s-form-item>
          </s-form>
        </s-collapse-item>
        <s-collapse-item title="选择脚本" name="script">
          <s-form :model="taskForm" :rules="rules1" ref="taskForm1" label-width="120px">
            <s-form-item label="脚本库" prop="scriptLibUuid" style="width: 100%">
              <input-tree v-model="taskForm.scriptLibUuid" :data="scriptLibraryList" :props="myProps" @select-change="getScriptList" style="width: 400px;" name="uuid"></input-tree>
            </s-form-item>
            <s-form-item label="脚本名称" prop="scriptUuidParList" style="width: 100%">
              <s-select v-model="taskForm.scriptUuidParList" @change="changeOwnType" collapse-tags multiple clearable style="width: 400px">
            ...
            <s-form-item label="超时时间" prop="timeOut">
              <s-input v-model="taskForm.timeOut" placeholder="秒，请输入正整数"></s-input>
            </s-form-item>
          </s-form>
        </s-collapse-item>
    ...
    methods: {
      ...
      getData() {
        ...
        if (this.current !== undefined) {
          ...
          this.ajaxObj.url = '/autoScriptTask/getRelationAssetList'
        } else {
          this.ajaxObj.url = '/autoMateItcomp/getCanAutoMateItcompInfo'
        }
