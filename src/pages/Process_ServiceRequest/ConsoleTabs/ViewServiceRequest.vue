      ...
      <s-scrollbar wrap-class="max-drawer-height">
        <s-form
          ...
          <s-form-item
            ...
            <!--文本-->
            <s-input
              v-if="item.htmlType==='udf_char_single_line' && serviceRequestUuid === '4c350009600748dfacaffd8a5734d244' && item.colnumName=='身份证号码'"
              :type="item.type?item.type:'text'"
              :disabled="!item.isSelected"
              :placeholder="item.placeholder?item.placeholder:''"
              :value="serviceRequestForm[item.uuid] | capitalizeIdCard(idCardFlag)" @input="handlerIdCard($event, item.uuid)">
            </s-input>
            <s-input
              v-else-if="item.htmlType==='udf_char_single_line' && serviceRequestUuid === '4c350009600748dfacaffd8a5734d244' && item.colnumName=='微信号'"
              :type="item.type?item.type:'text'"
              :disabled="!item.isSelected"
              :placeholder="item.placeholder?item.placeholder:''"
              :value="serviceRequestForm[item.uuid] | capitalizeUserWechat(userWechatFlag)" @input="handlerUserWechat($event, item.uuid)">
            </s-input>
            <s-input
              v-else-if="item.htmlType==='udf_char_single_line' && serviceRequestUuid === '4c350009600748dfacaffd8a5734d244' && item.colnumName=='个人地址'"
              :type="item.type?item.type:'text'"
              :disabled="!item.isSelected"
              :placeholder="item.placeholder?item.placeholder:''"
              :value="serviceRequestForm[item.uuid] | capitalizeUserAddress(userAddressFlag)" @input="handlerUserAddress($event, item.uuid)">
            </s-input>
            <s-input
              v-else-if="item.htmlType==='udf_char_single_line'"
              :type="item.type?item.type:'text'"
              :disabled="!item.isSelected"
              :placeholder="item.placeholder?item.placeholder:''"
            ...
            <!--数值-->
            <s-input
              v-if="item.htmlType==='udf_long' && serviceRequestUuid === '4c350009600748dfacaffd8a5734d244' && item.colnumName=='手机号码'"
              :disabled="!item.isSelected" type="number"
              :value="serviceRequestForm[item.uuid] | capitalizeUserCellphone(phoneFlag)" @input="handlerPhone($event, item.uuid)">
            </s-input>
            <s-input
              v-else-if="item.htmlType==='udf_long'"
              :disabled="!item.isSelected" type="number"
              v-model="serviceRequestForm[item.uuid]">
            </s-input>
            <!--下拉选项-->
...
<script type="text/babel">
  ...
  export default {
    ...
    methods: {
      handlerPhone(value, uuid) {
        if (this.phoneFlag && this.serviceRequestForm[uuid] !== '' && this.serviceRequestForm[uuid] !== null) {
          this.serviceRequestForm[uuid] = ''
          this.phoneFlag = false
        } else {
          this.serviceRequestForm[uuid] = value
        }
      },
      handlerUserWechat(value, uuid) {
        if (this.userWechatFlag && this.serviceRequestForm[uuid] !== '' && this.serviceRequestForm[uuid] !== null) {
          this.serviceRequestForm[uuid] = ''
      ...
      handlerUserAddress(value, uuid) {
        if (this.userAddressFlag && this.serviceRequestForm[uuid] !== '' && this.serviceRequestForm[uuid] !== null) {
          this.serviceRequestForm[uuid] = ''
      ...
      handlerIdCard(value, uuid) {
        if (this.idCardFlag && this.serviceRequestForm[uuid] !== '' && this.serviceRequestForm[uuid] !== null) {
          this.serviceRequestForm[uuid] = ''
          ...
          $axios.get(`/itilServiceRequest/getAttr/${this.serviceRequestUuid}`).then(({data}) => {
            ...
            // 构造表单绑定的数据对象
              ...
              data.forEach(item => {
                ...
                if (item.isInherent === 1) { // 自定义属性
                  this.$set(this.serviceRequestForm, item.uuid, item.defaultValue);
                }
                ...
                let checkInitNum = (rule, value, callback) => {
                  let reg = /^(([1-9][0-9]*)|(([0]\.\d{1,2}|[1-9][0-9]*\.\d{1,2})))$/
                  if (value) {
                    if (!reg.test(value)) {
                      callback(new Error('非负数，最多可包含小数点后两位'))
                    } else {
                      let y = 2147483648;
                      if (parseFloat(value) <= y) {
                        callback()
                      } else {
                        callback(new Error('请输入小于2147483648的数值'))
                      }
                    }
                  } else {
                    callback()
                ...
                if (item.htmlType === 'udf_long') {
                  if (item.isInherent === 1) { // 自定义数值
                    if (item.isRequired) {
                      if (item.colnumName === '手机号码') {
                        _this.serviceRequestFormRules[item.uuid].splice(0, 0, Validaters.Max(20), Validaters.MobilePhone, Validaters.NotNull);
                      } else {
                        this.serviceRequestFormRules[item.uuid].splice(0, 0, {validator: checkNum, trigger: 'blur'}, Validaters.NotNull);
                      }
                    } else {
                      if (item.colnumName === '手机号码') {
                        _this.serviceRequestFormRules[item.uuid].splice(0, 0, Validaters.Max(20), Validaters.MobilePhone);
                      } else {
                        this.serviceRequestFormRules[item.uuid].splice(0, 0, {validator: checkNum, trigger: 'blur'});
                      }
                    }
                  } else { // 内置数值
                    if (item.isRequired) {
                      if (item.colnumName === '手机号码') {
                        ...
                        this.serviceRequestFormRules[item.uuid].splice(0, 0, {validator: checkInitNum, trigger: 'blur'}, Validaters.NotNull);
                      }
                    } else {
                      if (item.colnumName === '手机号码') {
                        _this.serviceRequestFormRules[item.uuid].splice(0, 0, Validaters.Max(20), Validaters.MobilePhone);
                      } else {
                        this.serviceRequestFormRules[item.uuid].splice(0, 0, {validator: checkInitNum, trigger: 'blur'});
                      }
                    }
                  }
                  ...
                  if (item.isRequired) {
                    ...
                    this.serviceRequestFormRules[item.uuid].splice(1, 0, Validaters.NotNull);
                  } else {
                    this.serviceRequestFormRules[item.uuid].splice(0, 0, Validaters.Max(30));
                  }
                } else if (item.htmlType === 'udf_char_list') {
                  if (item.isRequired) {
                    this.serviceRequestFormRules[item.uuid].splice(0, 0, Validaters.SelectValNotNull);
                  }
                }
      ...
      selectChange(event, item) {
        ...
        _this.idCardFlag = true
        _this.userWechatFlag = true
        _this.userAddressFlag = true
        ...
        function save() {
          _this.$refs.serviceRequestForm.validate(valid => {
            if (valid) {
              ...
              // 0-保存，1-提交
              $axios.post(`/itilServiceRequest/saveApproveMapping/${uuid}/${saveType}`, values, {
                data: [],
                logTemplate: saveType === '0' ? `创建|服务请求(${_this.drawerTitle})` : `提交|服务请求(${_this.drawerTitle})`
              }).then(({data}) => {
    ...
    props: ['res'],
    filters: {
      capitalizeUserCellphone: (value, flag) => {
        if (!value) return ''
