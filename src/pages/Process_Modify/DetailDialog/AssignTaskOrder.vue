  ...
  export default{
    ...
    watch: {
      'assignTaskOrderForm.groupUuid'(newVal) {
        ...
        $axios.get(`/itilChangeOrder/getAllChangeChangeTaskAssignUserByGroup/${newVal}`).then(({data}) => {
    ...
    created() {
      $axios.get(`/itilChangeOrder/getAllChangeTaskAssignGroup/${this.companyUuid}`).then(({data}) => {
        if (data && data instanceof Array) {
