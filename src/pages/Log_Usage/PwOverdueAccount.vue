 ...
 * @author songyf
 * @date 2019/7/29
 * @Description: 【报表中心】→【账号使用情况统计】→【密码过期账号统计】
*/
<template>
  <div class="rules-operation" style="margin: 15px 10px;">
    ...
    <div style="margin: 15px 0 10px;">
      <s-button icon="psw-reset" title="重置密码" @click="resetPwd"  v-if="moduleType===0" v-noBtn="'accountManage_pwERepeatPw'">重置密码</s-button>
      <s-button icon="batch-del" title="批量删除" @click="bathDelete"  v-if="moduleType===0" v-noBtn="'accountManage_pwEBatchDel'">批量删除</s-button>
    ...
    <s-table-page
      ...
      <s-table-column type="selection" width="45" align="center"
                      v-if="moduleType===0 && judgeRoleBtn('accountManage_pwECheckBox')"></s-table-column>
      ...
      <s-table-column label="密码过期时间" prop="passwordExpirationTimeStr" show-overflow-tooltip></s-table-column>
      <s-table-column label="密码过期天数" prop="passwordExpirationDays" show-overflow-tooltip></s-table-column>
      <s-table-column label="账号状态" prop="statusStr" show-overflow-tooltip></s-table-column>
    </s-table-page>
    <s-dialog
      v-model="bloomSelect"
      v-if="bloomSelect"
    ...
    methods: {
      ...
      resetPwd() {
        ...
        if (this.batchUuid.length) {
          this.$confirm('确定重置密码?', '提示', {
            ...
            $axios.post(`/iamUserInfo/batchPasswordReset`, {userUuids: this.batchUuid}, {
              data: logData,
              logTemplate: '重置密码|密码过期账号统计>账号(#name#)'
            }).then(({data}) => {
              if (data) {
                this.$message({type: 'success', message: '重置密码成功，当前密码:p@ssw0rd'});
                this.getList()
              } else {
        ...
        if (this.batchUuid.length) {
          this.$confirm('确定删除?', '提示', {
            ...
            $axios.post(`/iamUserInfo/deleteUserInfoIncludeAccount`, this.batchUuid, {
              data: logData,
              logTemplate: '批量删除|密码过期账号统计>账号(#name#)'
            }).then(({data}) => {
              data.result = 'success';
              data.data.map(item => {
                if (!item.state) {
      ...
      DownloadHandle(type) {
        ...
        this.$confirm(`确定要以${type}格式导出？`, '提示', {
          ...
          $axios.post(`/iamUserInfo/passwordExpirationExport`, param).then(res => {
            if (res.data === 'success') {
              Download(`/iamUserInfo/downPasswordExpiration/${type}`)
            } else {
              this.$message({
                type: 'error',
                message: `${type}导出失败`
