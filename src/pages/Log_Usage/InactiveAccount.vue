 ...
 * @author songyf
 * @date 2019/7/29
 * @Description: 【报表中心】→【账号使用情况统计】→【不活跃账号统计】
*/
<template>
  <div class="rules-operation">
    ...
    <s-table-page
      ...
      <s-table-column label="不活跃天数" prop="day" show-overflow-tooltip></s-table-column>
      <s-table-column label="最后一次登录时间" prop="lastLoginTimeStr" show-overflow-tooltip></s-table-column>
    </s-table-page>
    <s-dialog
      v-model="bloomSelect"
      v-if="bloomSelect"
    ...
    methods: {
      ...
      DownloadHandle(type) {
        ...
        this.$confirm(`确定要以${type}格式导出？`, '提示', {
          ...
          $axios.post(`/iamUserInfo/inactiveAccountExport`, param).then(res => {
            if (res.data === 'success') {
              Download(`/iamUserInfo/downInactiveAccount/${type}`)
            } else {
              this.$message({
                type: 'error',
                message: `${type}导出失败`
