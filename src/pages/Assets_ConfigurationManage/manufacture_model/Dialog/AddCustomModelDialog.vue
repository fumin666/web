 ...
 * @Author: 高建鹏
 * @Version: //Q4厂商型号配置
 * @Description:型号tab编辑添加操作
 * @Position:配置项管理-》厂商型号配置-》型号页面-》编辑添加操作
...
<template>
  <s-dialog v-model="addDialog" :title="rowData ? '编辑' : '添加'" width="550px" :before-close="dialogClose">
    <s-form :model="attributeForm" :rules="rule" ref="attributeForm" label-width="80px">
      <s-form-item label="厂商" prop="manufactUuid" required>
        <s-select style="width: 100%" v-model="attributeForm.manufactUuid">
          <s-option v-for="(item) in manufactList" :key="item.uuid" :label="item.caption" :value="item.uuid"></s-option>
        </s-select>
      </s-form-item>
      <s-form-item label="型号" prop="caption">
  ...
  export default {
    ...
    data() {
      return {
        ...
        rule: {
          manufactUuid: [ Validaters.NotNull ],
    ...
    methods: {
      ...
      dialogOk() {
        this.$refs.attributeForm.validate((valid) => {
          ...
          let obj = {
            data: [],
            logTemplate: this.rowData ? `编辑|厂商型号配置>型号(${this.attributeForm.caption})` : `添加|厂商型号配置>型号(${this.attributeForm.caption})`
          }
          $axios.post('/resourcemanager/configurationmanage/manufactProduct/saveOrUpdateProduct', form, obj).then((res) => {
            if (res.data.state) {
