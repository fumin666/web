 ...
 * @Version: //Q4IP台账
 * @Description:扫描结果
 * @Position:IP台账-》子网视角-》IP地址扫描-》扫描结果
 * @Date: 2019-11-05 14:42:42
 * @LastEditors: mybells
 -->
<template>
  <s-dialog v-model="resultScanFlag" width="800px" title="IP地址扫描" :before-close="dialogClose">
    <s-row class="resultScanFlag_title">
      <i class="iconfont icon-zhengchang"></i>
      <span style="margin-left:60px;">扫描完成，共扫描<span class="font"> {{lastdata.ipAddressNum||0}} </span>IP地址</span>
    </s-row>
    <s-row class="step6">
      <s-col :span="7">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      lastdata() {
        return this.$store.state.addressScanSteps.lastdata;
      },
      dataList: {
        get() {
        ...
        set(val) {
          this.$store.commit('addressScanSteps/setStep1DataList', val);
        }
      }
    },
    mounted() {
    ...
    created() {
      ...
      this.$store.commit('addressScanSteps/setRunEndTime', new Date());
    },
    filters: {
      parseTime(val) {
