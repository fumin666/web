...
<script type="text/babel">
  ...
  import detailMixin from './PublishDetailMixin';
    ...
    methods: {
      ...
      getOrigin() {
        $axios.get('/itilChangeOrder/getChangeTaskSelectData').then((res) => {
          if (res.data) {
            ...
            this.form.taskSource = '6'
          }
        })
      },
      // 受派组
      assignGroup() {
        return $axios.get('/itilPublishOrder/getAllPublishTaskAssignGroup/' + this.form.companyUuid).then((res) => {
      ...
      assignUser() {
        if (this.form.assignGroup && this.assignGroupList.length > 0) {
          $axios.get(`/itilPublishOrder/getAllPublishTaskAssignUserByGroup/${this.form.assignGroup}`).then((res) => {
            if (res.data instanceof Array && res.data.length > 0) {
