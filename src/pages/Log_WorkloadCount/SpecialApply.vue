...
*/
<template>
  <section class="content logSpecialApply">
    <s-form :inline="true" class="search_form" label-width="80px">
      <div>
        ...
        <s-form-item label="运维类型：">
          <s-select v-model="specialTypeUuids" placeholder="请选择类型" clearable>
    ...
    <s-table-page
      ...
      <s-table-column label="运维类型" prop="specialoperTypeName"></s-table-column>
      <s-table-column label="申请次数" prop="applyNum"></s-table-column>
      <s-table-column label="资产数" prop="deviceCount"></s-table-column>
      <s-table-column label="账户数" prop="accountCount"></s-table-column>
      <s-table-column label="命令数" prop="commandCount"></s-table-column>
    </s-table-page>
    <!--点击选择机构-->
    <s-dialog
...
//        this.collectTask.taskPeriod = Number(this.collectTask.taskPeriod);
      ...
      showList() {
        let vm = this;
        $axios.post('/operationWorkStat/specialoperApplyCount', {departUuids: vm.institutionUuid, endTime: vm.endTimes, startTime: vm.beginTimes, specialTypeUuids: vm.specialTypeUuids}).then((res) => {
      ...
      DownloadHandle(type) {
        let vm = this;
        $axios.post('/operationWorkStat/makeSpecialApplyCountFile', {exportType: type, departUuids: vm.institutionUuid, endTime: vm.endTimes, startTime: vm.beginTimes, specialTypeUuids: vm.specialTypeUuids}).then((res) => {
          if (res.data) {
            Download(`/operationWorkStat/downSpecialApplyWorks/${type}`);
          }
        });
      },
      getOpsType() {
