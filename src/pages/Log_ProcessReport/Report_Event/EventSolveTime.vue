  ...
  export default {
    ...
    methods: {
      ...
      getData() {
        ...
        $axios.post(`/itilReportCenter/getIncidentReport/${2}`, params).then((res) => {
          this.tableDataA = res.data.statList
          this.tableDataB = res.data.detailList
          let eventObj = {};
          if (res.data.statchart && res.data.statchart.name.length > 0) {
            res.data.statchart.name.forEach((item, index) => {
              eventObj[item] = res.data.statchart.value[index]
            });
            this.chart1.data = eventObj;
            this.chart2.data = eventObj;
          }
        })
      },
      exportButton(name) {
        if (!this.startTime || !this.endTime) {
          ...
          this.$confirm('确定将事件平均解决时间报表以Excel格式导出？', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }).then(() => {
            $axios.post(`/itilReportCenter/exportItilReport`, param).then(res => {
              if (res.data === 'success') {
                Download(`/itilReportCenter/downItilReport/2/excel`)
              }
            })
          }).catch(() => {});
        }
