  ...
  export default {
    ...
    methods: {
      ...
      getData() {
        ...
        $axios.post(`/itilReportCenter/getChangeReport/${8}`, params).then((res) => {
      ...
      exportButton(name) {
        if (!this.startTime || !this.endTime) {
          ...
          this.$confirm('确定将变更积压数统计报表以Excel格式导出？', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }).then(() => {
            $axios.post(`/itilReportCenter/exportItilReport`, param).then(res => {
              if (res.data === 'success') {
                Download(`/itilReportCenter/downItilReport/8/excel`)
              }
            })
          }).catch(() => {});
        }
