    ...
    <s-table-page
      ...
      <s-table-column
        ...
        <template slot-scope="scope">
          <div :class="{'score-td': scope.row.logincount}" @click="showAccountDetail(scope.row.loginuseruuidList)">{{ scope.row.logincount }}</div>
        </template>
      </s-table-column>
      <s-table-column
        label="未登录">
        <template slot-scope="scope">
          <div :class="{'score-td': scope.row.unlogincount}" @click="showAccountDetail(scope.row.unloginuseruuidList)">{{ scope.row.unlogincount }}</div>
        </template>
      </s-table-column>
      <s-table-column
        label="账号总数"
        prop="">
        <template slot-scope="scope">
          <div>{{ scope.row.loginTotalCount }}</div>
        </template>
      </s-table-column>
    </s-table-page>
    <s-dialog
    ...
    methods: {
      ...
      ExcelDownloadHandle() {
        ...
        $axios.post('/iamLog/exportAccount/excel', param).then((res) => {
          if (res.data === 'success') {
            Download(`/iamLog/downAccount/login/excel?t=${new Date().getTime()}`);
          }
        });
      },
      PdfDownloadHandle() {
        ...
        $axios.post('/iamLog/exportAccount/pdf', param).then((res) => {
          if (res.data === 'success') {
            Download(`/iamLog/downAccount/login/pdf?t=${new Date().getTime()}`);
          }
        });
      },
      WordDownloadHandle() {
        ...
        $axios.post('/iamLog/exportAccount/word', param).then((res) => {
          if (res.data === 'success') {
            Download(`/iamLog/downAccount/login/word?t=${new Date().getTime()}`);
          }
        });
      }
    },
