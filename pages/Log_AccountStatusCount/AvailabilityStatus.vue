    ...
    <s-table-page
      ...
      <s-table-column
        ...
        <template slot-scope="scope">
          <div :class="{'score-td': scope.row.activecount}" @click="showAccountDetail(scope.row.activeuseruuidList)">{{ scope.row.activecount }}</div>
        </template>
      </s-table-column>
      <s-table-column
        label="未激活">
        <template slot-scope="scope">
          <div :class="{'score-td': scope.row.unactivecount}" @click="showAccountDetail(scope.row.unactiveuseruuidList)">{{ scope.row.unactivecount }}</div>
        </template>
      </s-table-column>
      <s-table-column
        label="锁定">
        <template slot-scope="scope">
          <div :class="{'score-td': scope.row.lockcount}" @click="showAccountDetail(scope.row.lockuseruuidList)">{{ scope.row.lockcount }}</div>
        </template>
      </s-table-column>
      <s-table-column
        label="过期">
        <template slot-scope="scope">
          <div :class="{'score-td': scope.row.expirycount}" @click="showAccountDetail(scope.row.expiryuseruuidList)">{{ scope.row.expirycount }}</div>
        </template>
      </s-table-column>
      <s-table-column
        label="离职">
        <template slot-scope="scope">
          <div :class="{'score-td': scope.row.leavecount}" @click="showAccountDetail(scope.row.leaveuseruuidList)">{{ scope.row.leavecount }}</div>
        </template>
      </s-table-column>
      <s-table-column
        label="待开启">
        <template slot-scope="scope">
          <div :class="{'score-td': scope.row.unopencount}" @click="showAccountDetail(scope.row.unopencountuuidList)">{{ scope.row.unopencount }}</div>
        </template>
      </s-table-column>
      <s-table-column
        label="注销">
        <template slot-scope="scope">
          <div :class="{'score-td': scope.row.writeoffcount}" @click="showAccountDetail(scope.row.writeoffcountuuidList)">{{ scope.row.writeoffcount }}</div>
        </template>
      </s-table-column>
      <s-table-column
        label="账号总数"
        prop="">
        <template slot-scope="scope">
          <div>{{ scope.row.avaTotalCount }}</div>
        </template>
      </s-table-column>
    </s-table-page>
    <s-dialog
    ...
    methods: {
      ...
      ExcelDownloadHandle() {
        ...
        $axios.post('/iamLog/exportAccount/excel', param).then((res) => {
          if (res.data === 'success') {
            Download(`/iamLog/downAccount/ava/excel?t=${new Date().getTime()}`);
          }
        });
      },
      PdfDownloadHandle() {
        ...
        $axios.post('/iamLog/exportAccount/pdf', param).then((res) => {
          if (res.data === 'success') {
            Download(`/iamLog/downAccount/ava/pdf?t=${new Date().getTime()}`);
          }
        });
      },
      WordDownloadHandle() {
        ...
        $axios.post('/iamLog/exportAccount/word', param).then((res) => {
          if (res.data === 'success') {
            Download(`/iamLog/downAccount/ava/word?t=${new Date().getTime()}`);
          }
        });
      }
    },
