...
import { timeStamp, parseNumAllTime, intersectObj, Validaters } from 'sunflower-common-utils';
    ...
    'bulletinForm.typeUuid': {
      ...
      handler(val) {
        if (val) {
          ...
          this.$refs['customRef'].fileList = []
          if (this.safetyInspectionData !== null) { // 如果是从等保安全检查进来
            this.$refs['customRef'].fileList.push({
              url: this.safetyInspectionData.filePath,
              name: this.safetyInspectionData.filePath,
              safeAttachmentUuid: this.safetyInspectionData.safeAttachmentUuid
            })
          }
        }
      }
  ...
  methods: {
    ...
    getOptionData() {
      $axios.get(`/safetyPlatform/bulletinIssuedTask/toCreateBulletinIssuedTask`).then(({data}) => {
        this.safeRoleList = data.safeRoleList
        this.safeTypeList = data.safeTypeList.filter(v => {
          if (v.isDelete !== 1) {
            return v
          }
        })
        if (this.$route.meta.name === '等保工作') {
          this.safeTypeList = this.safeTypeList.filter(v => {
    ...
    getUserList() { // 人员列表
      ...
      if (this.$route.meta.name === '安全通报任务') {
        params.roleType = 2
      } else {
        params.roleType = 1
      }
                  ...
                  if (valid3) {
                    vm.$refs['noticeForm'].validate((valid4) => {
                      if (valid4) {
                        ...
                        let obj = {
                          logTemplate: Object.keys(this.rowData).length !== 0 ? `编辑|通报(${attrValue})` : `下发|通报(${attrValue})`
                        }
