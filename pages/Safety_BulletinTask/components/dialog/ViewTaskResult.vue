...
<script>
  ...
  import AddEditKnowledge from '@/pages/SafetyBulletin_Knowledge/Dialog/AddEditKnowledge'
  export default {
    components: {
      viewTaskFile,
      AddEditKnowledge
    ...
    methods: {
      getList() {
        let vm = this;
        if (vm.historyType) { // 根据类型判断是从历史任务进来
          vm.taskList = vm.resultData;
          vm.taskListNew = vm.resultData;
          vm.currentItemArr = vm.taskList.length !== 0 ? vm.taskList[0].workflows : [];
          vm.workflowFileList = vm.taskList.length !== 0 ? vm.taskList[0].workflowFileList : [];
          vm.fileCount = vm.taskList.length !== 0 ? vm.taskList[0].fileCount : 0;
          vm.currentItem = 0
        } else {
