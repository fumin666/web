  ...
  <s-dialog v-model="dialogFlag" title="查看任务执行历史记录" width="800px" append-to-body class="senior-box">
    <s-scrollbar wrap-class="default-dialog-scroll">
      <s-form :inline="true" :model="searchForm" class="search_form" style="margin:10px 0" label-width='120px'>
        <div>
          ...
          <s-form-item label="状态：">
            <s-select v-model="searchForm.singleStatus" clearable>
      ...
      <RealPageTable
        ...
        <s-table-column prop="theme" label="通报主题" show-overflow-tooltip>
        </s-table-column>
...
<script>
  ...
  export default {
    ...
    filters: {
      filterTime(val) {
        return val ? parseNumAllTime(val) : '-'
      },
      filterStatus(val) {
        return val === 0 ? '进行中' : '已完成'
      }
    },
    created() {
      this.$nextTick(() => {
    ...
    methods: {
      ...
      delAxios(arr) {
        ...
        if (arr instanceof Array) { // 如果是批量删除
          ...
          if (arr.singleStatus === 0) {
            isFlag = false
          }
        }
        if (!isFlag) {
          this.$message({message: '尚有任务正在执行中，不支持删除操作!', type: 'warning'});
          return
        }
