 ...
 * @Date: 2020-1-19
 * @LastEditors: songyf
 * @Description: 安全通报-安全通报任务-通报下发
 -->
<template>
  <s-dialog v-model="dialogFlag" :title="Object.keys(rowData).length !==0 ?'编辑':'下发通报'" width="750px" append-to-body
            class="release-bulletin">
      ...
      <s-collapse accordion class="" v-model="activeNames">
        ...
        <s-collapse-item title="通报任务" name="bulletinTask">
          <s-form :model="bulletinForm" ref="taskForm" :rules="bulletinRules" label-width="135px">
            <s-form-item label="是否周期任务" class="required-tips">
              <s-radio-group v-model="bulletinForm.cycleTaskStatus" :disabled="isCycle">
            ...
            <s-form-item label="下发时间" prop="startTime">
              <s-date-picker style="width: 250px"
                             ...
                             :editable="false"
                             type="datetime"
                             placeholder="选择日期"></s-date-picker>
              <div style="float: left;margin-left: 8px; line-height: 36px">
                <s-checkbox v-model="immediatelyIssued">立即下发</s-checkbox>
              </div>
            </s-form-item>
            <s-form-item label="启动时间" prop="orderStartTime">
              <s-date-picker style="width: 250px"
                             ...
                             placeholder="选择日期"></s-date-picker>
              <div style="float: left;margin-left: 8px; line-height: 36px">
                <s-checkbox v-model="immediatelyStart">立即启动</s-checkbox>
              </div>
            </s-form-item>
            <s-form-item label="结束时间" prop="endTime">
              ...
              <div style="float: left;margin-left: 8px; line-height: 36px" v-if="bulletinForm.cycleTaskStatus === 1">
                <s-checkbox v-model="nerverEnd">永不结束</s-checkbox>
              </div>
            </s-form-item>
            <s-form-item label="单次任务持续天数" prop="onceTaskTime" v-if="bulletinForm.cycleTaskStatus === 1">
              <s-input v-model="bulletinForm.onceTaskTime"></s-input>
            </s-form-item>
          </s-form>
        </s-collapse-item>
        <s-collapse-item title="通报对象" name="bulletinObject">
          <s-form :model="bulletinForm" ref="objectForm" :rules="bulletinRules" label-width="135px">
            <s-form-item label="下发方式" prop="excutorType" class="required-tips">
              <s-radio-group v-model="bulletinForm.excutorType">
            ...
            <s-form-item label="选择部门" prop="departUuidList" v-if="bulletinForm.excutorType === 1">
              <s-select v-model="bulletinForm.departUuidList" multiple collapse-tags filterable clearable>
            ...
            <s-form-item label="选择角色" prop="roleUuidList" v-if="bulletinForm.excutorType === 2">
              <s-select v-model="bulletinForm.roleUuidList" multiple collapse-tags filterable clearable>
            ...
            <div v-if="bulletinForm.excutorType === 3">
              <s-input placeholder="请输入搜索内容" v-model="searchKey" class="real-search-input"
                       icon="magnifier" @input="getUserList"></s-input>
              <RealPageTable
                :ajax-obj="ajaxObj"
        ...
        <s-collapse-item title="任务启动/结束邮件通知" name="bulletinNotice">
          <s-form :model="bulletinForm" ref="noticeForm" :rules="bulletinRules" label-width="135px">
            ...
            <s-form-item label="提前通知时间" prop="safeTaskEmailNotice.startValue"
                         v-if="bulletinForm.safeTaskEmailNotice.startNoticeStatus === 1">
              <s-input v-model="bulletinForm.safeTaskEmailNotice.startValue" style="width: 200px"></s-input>
              <s-select v-model="bulletinForm.safeTaskEmailNotice.startUnit" style="width: 200px; margin-left: 20px">
            ...
            <s-form-item label="提前通知时间" prop="safeTaskEmailNotice.endValue"
                         v-if="bulletinForm.safeTaskEmailNotice.endNoticeStatus === 1">
              <s-input v-model="bulletinForm.safeTaskEmailNotice.endValue" style="width: 200px"></s-input>
              <s-select v-model="bulletinForm.safeTaskEmailNotice.endUnit" style="width: 200px; margin-left: 20px">
                <s-option label="天" :value="1"></s-option>
                <s-option label="时" :value="2"></s-option>
                <s-option label="分" :value="3"></s-option>
              </s-select>
    ...
    <template slot="footer" class="dialog-footer">
      <s-button @click="saveRelease">确定</s-button>
