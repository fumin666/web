<template>
  <section id="workInfoDetailCtn" wrap-style="max-height: 450px;">
    ...
    <s-row>
      ...
      <s-col span="10" class="grid-content bg-purple">工作信息：</s-col>
      <s-col span="14">
        <s-tooltip :content="detailData.logDetail">
          <div class="grid-content bg-purple-light overflowTxt">{{detailData.workInfo}}</div>
        </s-tooltip>
      </s-col>
    </s-row>
  ...
  export default{
    ...
    methods: {
      ...
      downloadAttachment(row) {
        Download(`/safetyPlatform/fileUpload/downloadOrderTaskFile/${row.fileUuid}/2`);
      },
      // 删除附件
