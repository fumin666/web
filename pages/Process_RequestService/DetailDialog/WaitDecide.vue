    ...
    methods: {
      saveWaitDecide() {
        this.$refs.waitDecideForm.validate((valid) => {
          if (valid) {
            $axios.post(`/itilServiceRequest/requestOrderPendens`, {requestOrderUuid: this.orderUuid, pendens: Number(this.waitDecideForm.pendingReason)}).then(({data}) => {
              if (data.state === true) {
                this.$message({
                  type: 'success',
                  message: '待决操作成功！'
