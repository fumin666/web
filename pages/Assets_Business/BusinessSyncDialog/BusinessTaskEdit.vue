    ...
    <s-collapse v-model="activeName" accordion>
      ...
      <s-collapse-item title="同步对象" name="2">
        <s-table-page
          ...
          <s-table-column label="业务名称" prop="bSystemName">
          </s-table-column>
        </s-table-page>
      </s-collapse-item>
  ...
  export default {
    ...
    methods: {
      ...
      defSearch() {
        return {
          width: 10,
          placeholder: '请输入账号名称、业务名称查询',
          searchProps: ['name', 'bSystemName']
        }
      },
    ...
    created() {
      if (this.type === 'see') {
        ...
        this.isShow = true;
        $axios.get('/iamBusinessAccount/getAllBusinessAccount').then(res => {
          if (res.status === 200) {
