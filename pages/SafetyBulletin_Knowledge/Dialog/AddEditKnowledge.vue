 ...
 * @Author: mybells
 * @Version://Q1知识库
 * @Description:安全通报->知识库->添加编辑src\home\ProcessCenter\ProcessCreatOrder\KnowDialog.vue
    ...
    methods: {
      ...
      getKnowledgeTypeList() {
        $axios.get('safetyPlatform/safetyBulletin/getUndeleteSafeType').then(res => {
          this.knowledgeTypeSelect = res.data || [];
          if (this.knowledgeTypeSelect.length) { this.form.safeTypeUuid = this.tabKey; }
        })
      },
      // 附件信息文件上传成功回调
      ...
      save() {
        if (this.isAdd) {
          let form = Object.assign({onlyGradeProtectShow: 0, createBy: this.$store.getters.userData.uuid}, this.form)
