<template>
  <div class="change1">
    ...
    <div class="down-content print_field noShow">
      <section ref="print">
        <h1 class="noShow">按变更路径统计变更总数</h1>
        <s-card ref="card" style="margin-bottom: 20px">
          <div style="display: flex">
            <div style="width: 400px">
  ...
  export default {
    ...
    methods: {
      ...
      getData() {
        ...
        $axios.post(`/itilReportCenter/getChangeReport/${7}`, params).then((res) => {
      ...
      exportButton(name) {
        if (!this.startTime || !this.endTime) {
          ...
          this.$confirm('确定将按变更路径统计变更总数报表以Excel格式导出？', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }).then(() => {
            $axios.post(`/itilReportCenter/exportItilReport`, param).then(res => {
              if (res.data === 'success') {
                Download(`/itilReportCenter/downItilReport/7/excel`)
              }
            })
          }).catch(() => {});
        }
