  ...
  <div class="change1">
    <section class="reportTop noPrint">
      ...
      <s-table
        ...
        <s-table-column prop="taskNum" label="任务单数" show-overflow-tooltip></s-table-column>
        <s-table-column prop="taskTotalTime" label="任务时长" show-overflow-tooltip></s-table-column>
        <s-table-column prop="changNum" label="变更单数" show-overflow-tooltip></s-table-column>
        <s-table-column prop="changTotalTime" label="变更时长" show-overflow-tooltip></s-table-column>
      </s-table>
    </section>
...
//            ]
    ...
    methods: {
      getData() {
        ...
        $axios.post(`/itilReportCenter/getPublishReport2/${14}`, params).then((res) => {
          ...
          this.chart.data = newArr;
          this.chart1.data = newArr;
        })
      },
      filterTableData(obj) {
        let typeName = obj.seriesName;
        for (let i in this.publishStage) {
          if (this.publishStage[i] === typeName) {
            this.typeNum = i;
          }
        }
        if (this.typeNum) {
          ...
          for (let i in this.filterData) {
            if (this.filterData[i].publishStage === parseInt(this.typeNum)) {
      ...
      exportButton() {
        if (!this.startTime || !this.endTime) {
          ...
          this.$confirm('确定将发布管理平均处理时长报表以Excel格式导出？', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }).then(() => {
            $axios.post(`/itilReportCenter/exportItilReport`, param).then(res => {
              if (res.data === 'success') {
                Download(`/itilReportCenter/downItilReport/14/excel`)
              }
            })
          }).catch(() => {});
        }
