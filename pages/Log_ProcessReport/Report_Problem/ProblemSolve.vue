  ...
  export default {
    ...
    methods: {
      ...
      getData() {
        ...
        $axios.post(`/itilReportCenter/getProblemReport/${6}`, params).then((res) => {
      ...
      exportButton(name) {
        if (!this.startTime || !this.endTime) {
          ...
          this.$confirm('确定将问题成功解决情况统计报表以Excel格式导出？', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }).then(() => {
            $axios.post(`/itilReportCenter/exportItilReport`, param).then(res => {
              if (res.data === 'success') {
                Download(`/itilReportCenter/downItilReport/6/excel`)
              }
            })
          }).catch(() => {});
        }
