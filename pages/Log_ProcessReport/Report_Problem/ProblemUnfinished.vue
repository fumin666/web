<template>
  <div class="problem">
    ...
    <div class="down-content print_field noShow">
      <section ref="print">
        <h1 class="noShow">未完成详情内容分类统计</h1>
        <s-card ref="card" style="margin-bottom: 20px">
          <div style="display: flex">
            <div style="width: 400px">
  ...
  export default {
    ...
    methods: {
      ...
      getData() {
        ...
        $axios.post(`/itilReportCenter/getProblemReport/${5}`, params).then((res) => {
      ...
      exportButton(name) {
        if (!this.startTime || !this.endTime) {
          ...
          this.$confirm('确定将未完成问题情况统计报表以Excel格式导出？', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }).then(() => {
            $axios.post(`/itilReportCenter/exportItilReport`, param).then(res => {
              if (res.data === 'success') {
                Download(`/itilReportCenter/downItilReport/5/excel`)
              }
            })
          }).catch(() => {});
        }
