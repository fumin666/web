  ...
  export default {
    ...
    methods: {
      ...
      getData() {
        ...
        $axios.post(`/itilReportCenter/getRequestReport/${17}`, params).then(({data}) => {
          this.chart1.data = data.percent.split('%')[0]
          this.chart2.data = data.percent.split('%')[0]
          this.chart1.itemName = '服务请求成功解决率'
          this.chart2.itemName = '服务请求成功解决率'
          this.tableDataA = [{count: data.count, success: data.success}]
      ...
      exportButton() {
        if (!this.startTime || !this.endTime) {
          ...
          this.$confirm('确定将服务请求成功解决率报表以Excel格式导出？', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }).then(() => {
            $axios.post(`/itilReportCenter/exportItilReport`, param).then(res => {
              if (res.data === 'success') {
                Download(`/itilReportCenter/downItilReport/17/excel`)
              }
            })
          }).catch(() => {});
        }
