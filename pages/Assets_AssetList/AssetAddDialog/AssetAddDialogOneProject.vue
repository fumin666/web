/**
* Created by dgunzi on 2019/05/21.
* 资产添加页面（单独注册OMA）
*/
<!--
 * @Author: 高建鹏
 * @Version: //Q4资产添加/编辑
 * @Description:资产添加编辑（单独注册OMA）
 * @Position:资产列表-》添加/编辑
...
<template>
<s-scrollbar wrap-style="maxHeight:650px;">
  <section id="addAssetFormCtnOMA" style="padding-right:10px;">
      ...
      <div class="wrapper">
        ...
        <div class="panel">
          <s-form-item label="IP地址" prop="monitorItcomp.controlAdderss">
            <s-input v-model="addAssetForm.monitorItcomp.controlAdderss">
                <s-button slot="suffix" size="mini" title="IP地址分配" @click="allocationIP" v-if="!isApplication">分配IP</s-button>
            </s-input>
          </s-form-item>
        </div>
        <div class="panel">
        ...
        <div class="panel">
          <s-form-item label="厂商" prop="monitorItcomp.manufactUuid">
            <s-select v-model="addAssetForm.monitorItcomp.manufactUuid" @clear="emptyManufact" filterable>
              <s-option
        ...
        <div class="panel">
          <s-form-item label="型号" prop="monitorItcomp.productUuid">
            <s-select v-model="addAssetForm.monitorItcomp.productUuid" @clear="emptyProduct" @change="productChange" filterable>
              <s-option
        ...
        <div class="panel">
          <s-form-item label="配置管理员机构" prop="monitorItcomp.operationOrguuid">
            <s-select
              ...
              @change="handleOperationOrgChange">
              <s-option
        ...
        <div class="panel">
          <s-form-item label="配置管理员" prop="monitorItcomp.maintenanceStaffuuid">
            <s-select v-model="addAssetForm.monitorItcomp.maintenanceStaffuuid">
              <s-option
        ...
        <div class="panel">
          <s-form-item label="公司名称" prop="monitorItcomp.firmUuid">
            <s-select v-model="addAssetForm.monitorItcomp.firmUuid" @clear="emptyFirmUuid" clearable>
              <s-option
        ...
        <div class="panel">
          <s-form-item label="使用部门" prop="monitorItcomp.cmdbOrguuid">
            <s-select v-model="addAssetForm.monitorItcomp.cmdbOrguuid" @clear="emptyCmdbOrguuid" clearable>
              <s-option
        ...
        <div class="panel" v-if="inDataObj['所在机柜']">
          <s-form-item label="所在机柜" prop="monitorItcomp.atCabinetNumber">
            <s-input v-model="addAssetForm.monitorItcomp.atCabinetNumber"></s-input>
          </s-form-item>
        </div>
        ...
        this.$refs.addAssetForm.validate(valid => {
          if (valid) {
            ...
            if (this.formType === 'add') {
              this.$emit('addPost', saveData);
            } else {
              this.$emit('editPost', saveData);
            }
          } else {
    ...
    created() {
      ...
      this.getInitData().then(() => {
        ...
        if (this.formType === 'add') {
          ...
          if (this.addAssetForm.monitorItcomp.citypeUuid === null) {
            this.addAssetForm.monitorItcomp.citypeUuid = []
          }
        } else if (this.formType === 'edit') {
          this.assetUuid = this.assetUuidProp
          // "编辑"操作
          this.getDetailData(this.assetUuidProp);
        }
