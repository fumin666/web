 ...
 * @LastEditTime: 2020-08-07 16:56:02
 * @LastEditors: mybells
 -->
<!-- 资产账户 -->
<template>
  <div class="accountList">
    <s-row class="left-btn" v-show="!assetName" style="margin-bottom:10px;">
      <s-button icon="plus" @click="add" v-if="judgeRoleBtn('assetsManage_addAssetAccount') || judgeRoleBtn('assetsManage_omaAddAssetAccount')">添加</s-button>
      <s-button icon="batch-del" @click="del" v-if="judgeRoleBtn('assetsManage_batchDelAssetAccount') || judgeRoleBtn('assetsManage_omaBatchDelAccount')">批量删除</s-button>
      <s-button @click="batchImport" icon="import" v-if="judgeRoleBtn('assetsManage_importAssetAccount') || judgeRoleBtn('assetsManage_omaImportAssetAccount')">批量导入</s-button>
      <s-button @click="batchExport" icon="export" v-if="judgeRoleBtn('assetsManage_exportAssetAccount') || judgeRoleBtn('assetsManage_omaExportAssetAccount')">批量导出</s-button>
      <s-button @click="batchAsync" icon="union" v-if="judgeRoleBtn('assetsManage_batchSysnAssetAccount') || judgeRoleBtn('assetsManage_omaBatchSysnAssetAccount')">批量同步</s-button>
    </s-row>
    ...
    <account-add-dialog v-if="addDialogFlag" v-model="addDialogFlag" ref="addAccount" :uuid='assUuid' :asset-uuid="assetUuid" :cur-account="curAccount"
    ...
    <s-dialog v-model="changePwdDialog" v-if="changePwdDialog" title="手动改密" width="650px" ref="changePwdDialog"  append-to-body>
      <account-change-pwd :account="curAccount" @closeActChgPwdDialog="closeActChgPwdDialog"></account-change-pwd>
    </s-dialog>
    <import-dialog v-model="importOrgDialog" v-if="importOrgDialog" :uploadObj="uploadObj" :downObj="downObj" :fileTypes='fileType'
      ...
      rowAssociation(row) { // 打开关联窗口
        ...
        $axios.get(`/linkAccount/getAllDatabaseProtoclByAccount/${row.uuid}`).then((res) => {
          if (res.data.length > 0) {
            ...
            this.$message({showClose: true, message: '请先到"协议设置"页签进行数据维护再进行关联操作！', type: 'warning'});
          }
        });
      },
      // 同步
