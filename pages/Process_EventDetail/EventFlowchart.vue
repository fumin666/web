  ...
  <div class="process_graph_main">
    <div class="process-title-line">
      <div class="process-title">
        <div class="title-info">{{`事件单${this.$route.params.serialCode}流程图`}}</div>
      ...
      <div class="g-graph-content">
        <div class="s-steps is-horizontal" id="graphTableTitle">
          ...
          <div class="s-step otherType" id="step_12">
            <div class="s-step-main">
              <div class="s-step-title">调查与分派</div>
            </div>
          </div>
          <div class="s-step otherType" id="step_13">
            <div class="s-step-main">
              <div class="s-step-title">诊断与恢复</div>
            </div>
          </div>
    ...
    <div class="parent" id="main-parent">
      ...
      <div class="g-graph-content">
        ...
        <div class="line">
          <div class="g-cell" id="cell41">
            <div class="r-line"></div>
          </div>
          <div class="g-cell" id="cell42">
            <div class="r-line"></div>
          </div>
          <div class="g-cell" id="cell43">
            <div class="r-line"></div>
          </div>
          <div class="g-cell" id="cell44">
            <div class="r-line"></div>
          </div>
          <div class="g-cell" id="cell45"></div>
        </div>
      </div>
    </div>
  ...
  methods: {
    ...
    insertCellFromData() {
      ...
      try {
        ...
        let preVertex, edge;
        if (dataLength > 0) {
          // let geo = _self.getCellGeo(Object.keys(this.graphData[0])[0], 1, 1);
          // let vertex;
          // if (dataLength === 1) {
          //   vertex = graph.insertVertex(parent, null, this.getContentHtml(Object.keys(this.graphData[0])[0], true), geo.x, geo.y, _self.vertexWidth, _self.vertexHeight);
          // } else {
          //   vertex = graph.insertVertex(parent, null, this.getContentHtml(Object.keys(this.graphData[0])[0], false), geo.x, geo.y, _self.vertexWidth, _self.vertexHeight);
          // }
          // vertex.text = Object.keys(this.graphData[0])[0]
          // vertexAry.push(vertex);

          /* *** 已下判断是5.29朱金奇需要添加，有问题找金奇*** */
          let geo = _self.getCellGeo(Object.keys(this.graphData[0])[0], 1, 1);
          let vertex;
          if (dataLength === 1) {
            this.graphData.push({ '已指派': this.graphData[0][Object.keys(this.graphData[0])[0]] });
            vertex = graph.insertVertex(parent, null, this.getContentHtml(Object.keys(this.graphData[0])[0], false), geo.x, geo.y, _self.vertexWidth, _self.vertexHeight);
            vertex.text = Object.keys(this.graphData[0])[0]
            vertexAry.push(vertex);
            geo = _self.getCellGeo(Object.keys(this.graphData[1])[0], 2, 2);
            vertex = graph.insertVertex(parent, null, this.getContentHtml(Object.keys(this.graphData[1])[0], true), geo.x, geo.y, _self.vertexWidth, _self.vertexHeight);
            vertex.text = Object.keys(this.graphData[1])[0]
    ...
    getCellInfo(graph, cellText, x, y, origin) {
      ...
      if (!active) {
        ...
        tipDiv.innerHTML = `<div class="info-title"><i class="iconfont icon-tag"></i>处理人信息</div><div class="line">姓名：${active.userRealName || '无'}</div><div class="line">邮箱：${active.userMail || '无'}</div><div class="line">电话：${active.userCellphone || '无'}</div>`;
      }
      //   let body = '';
    ...
    getFlowData(uuid) {
      $axios.get(`/itilIncidentCenter/getFlowChart/${uuid}`).then(({ status, data }) => {
        if (status === 200 || status === 304) {
          this.graphData = data.history
          this.initGraph();
        } else {
          this.$message({
            type: 'error',
