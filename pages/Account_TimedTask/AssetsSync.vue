 ...
 * @Position:
 -->
<template>
  <section class="fileList" style="margin: 20px">
    <div v-if="notResultDetail">
      <s-form :inline="true" class="search_form" style="margin:10px 0" label-width='105px'>
        <div>
          <s-form-item label="任务名称：">
            <s-input v-model="TaskSearchForm.taskName"></s-input>
          </s-form-item>
          <s-form-item label="创建者：">
            <s-input v-model="TaskSearchForm.createUserName"></s-input>
          </s-form-item>
          <s-form-item label="任务周期：">
            <s-select v-model="TaskSearchForm.taskType" filterable clearable>
              <s-option label="否" :value="0"></s-option>
              <s-option label="是" :value="1"></s-option>
            </s-select>
          </s-form-item>
          <s-form-item label="状态：" label-width='100px'>
            <s-select v-model="TaskSearchForm.taskStatus" filterable clearable>
      ...
      <s-table-page
        ...
        <s-table-column label="操作" width="190">
          <template slot-scope="scope">
            ...
            <i class="iconfont icon-edit" title="编辑定时任务" @click="editTask(scope.row)" v-if="scope.row.taskStatus!=='已删除'"> </i>
            <i class="iconfont icon-account-sync" title="查看同步结果" @click="scanResult(scope.row)"></i>
    ...
    <div v-else>
      <AssetsSyncResultDialog ref="syncResult" :syncResultList="syncResultList" @back="notResultDetail=true"></AssetsSyncResultDialog>
    </div>
    <!--新增资产账号定时同步任务-->
    <s-dialog
      v-model="taskAdd"
      ...
      width="950px"
      title="新增同步任务">
      <Assets-task-add-dialog ref="taskAdd" :emaillist="selectedEmailList" @showEmailM="bloomMyEmail = true" @showEmail="bloomLink = true"></Assets-task-add-dialog>
    ...
    <s-dialog
      ...
      width="950px"
      title="编辑同步任务">
      <Assets-task-edit-dialog ref="taskEdit" :defaultData="rowData" :emaillist="selectedEmailList"  @showEmailM="bloomMyEmail = true" @showEmail="bloomLink = true"></Assets-task-edit-dialog>
      <template slot="footer" class="dialog-footer">
    ...
    <s-dialog
      ...
      width="950px"
      title="查看同步任务">
      <Assets-task-edit-dialog ref="taskSee" type="see" :defaultData="rowData" :emaillist="selectedEmailList"></Assets-task-edit-dialog>
    </s-dialog>
    <!--手动增加邮件-->
    <s-dialog
      ...
      v-if="bloomMyEmail"
      title="手动添加邮件">
      <Assets-add-email-m ref="addMyEmail"></Assets-add-email-m>
      <template slot="footer" class="dialog-footer">
    ...
    <s-dialog
      ...
      <s-scrollbar wrap-style="max-height: 600px;">
        <Assets-add-email ref="linkEmail" :data="selectedEmailList"></Assets-add-email>
      </s-scrollbar>
      <template slot="footer" class="dialog-footer">
...
<script>
  ...
  import AssetsTaskAddDialog from './AssetsSyncDialog/AssetsTaskAdd.vue'
  import AssetsTaskEditDialog from './AssetsSyncDialog/AssetsTaskEdit.vue'
  import AssetsSyncResultDialog from './AssetsSyncDialog/AssetsSyncResult.vue'
  import AssetsAddEmailM from './AssetsSyncDialog/AssetsAddEmailM'
  import AssetsAddEmail from './AssetsSyncDialog/AssetsAddEmail'
  ...
  export default {
    ...
    components: {
      // SearchBox,
      AssetsTaskAddDialog,
      AssetsTaskEditDialog,
      AssetsSyncResultDialog,
      AssetsAddEmailM,
      AssetsAddEmail,
      DateRange
    },
      ...
      addOk() {
        ...
        this.$refs['taskAdd'].$refs['baseForm'].validate((valid) => {
          ...
          this.$refs['taskAdd'].$refs['period'].$refs['periodForm'].validate((valid) => {
            ...
            $axios.post('/commonTask/addTask', data, {
              data: [{name: commonTask.taskName}],
              logTemplate: '添加|同步任务(#name#)'
            }).then((res) => {
              if (res.data.state) {
                  this.$message({
                    message: '保存成功',
      ...
      editOk () {
        ...
        this.$refs['taskEdit'].$refs['baseForm'].validate((valid) => {
          ...
          this.$refs['taskEdit'].$refs['period'].$refs['periodForm'].validate((valid) => {
            ...
            $axios.post('/commonTask/updateTask', data, {
              data: [{name: commonTask.taskName}],
              logTemplate: '编辑|同步任务(#name#)'
            }).then((res) => {
              if (res.data.state) {
                  this.$message({
                    message: '编辑成功',
