      ...
      customData: {
        ...
        handler(val) {
          ...
          this.customData.forEach((v) => {
            ...
            if (v.isRequired === 1) {
              if (v.attrType === 3) {
                vm.$set(vm.customRule, v.uuid, vm.customRule[v.uuid] ? [...vm.selectRequire, ...vm.staffRule[v.propertyKey]] : vm.selectRequire)
              } else {
                vm.$set(vm.customRule, v.uuid, vm.customRule[v.uuid] ? [...vm.textRequire, ...vm.textMax, ...vm.staffRule[v.propertyKey]] : [...vm.textRequire, v.attrType !== 2 ? vm.textMax : vm.textAreaMax])
              }
            } else {
              if (v.attrType !== 3) {
                vm.$set(vm.customRule, v.uuid, vm.customRule[v.uuid] ? [...vm.textMax, ...vm.staffRule[v.propertyKey]] : v.attrType !== 2 ? vm.textMax : vm.textAreaMax)
              }
            }
          })
