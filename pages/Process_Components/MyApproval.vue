<template>
  <div style="margin:10px 5px 0 5px;" class="myother">
    <s-form :inline="true" :model="searchForm" class="search_form">
      <div>
        <s-form-item label="审批状态:">
          <s-select v-model="searchForm.approveResult" style="width: 185px">
        ...
        <s-form-item label="工单类型:">
          <s-select v-model="searchForm.orderType" style="width: 185px">
    ...
    <div style="margin: 10px 0;">
      <s-button @click="batchallow" :disabled="allowButton" icon="info-read">通过</s-button>
      <s-button @click="batchrefuse" :disabled="refuseButton" icon="cancel">拒绝</s-button>
    </div>
    <s-table-page :data="list"
      ...
      <s-table-column label="操作">
        <template slot-scope="scope">
          <i :class="'iconfont icon-info-read ' + (disabled ? 'disabled': '')" :disabled="delIsShow(scope.row)" @click="allow(scope.row, disabled)" title="通过"></i>
          <i :class="'iconfont icon-cancel ' + (disabled ? 'disabled': '')"  @click="refuse(scope.row, disabled)" title="拒绝"></i>
        </template>
      </s-table-column>
    </s-table-page>
    <s-dialog
...
<script>
  ...
  import CloseDialog from '../Process_MyOther/components/CloseDialog';
    ...
    methods: {
      ...
      search() {
        ...
        let params = {
          ...
          commitTimeStart: this.searchForm.commitTimeStart ? parseInt(new Date(this.searchForm.commitTimeStart).getTime() / 1000) : null,
          commitTimeEnd: this.searchForm.commitTimeEnd ? parseInt(new Date(this.searchForm.commitTimeEnd).getTime() / 1000) : null
        }
      ...
      dialogallow(params) {
        this.$confirm('确定通过?', '提示', {
          ...
          let uuid = params.map(item => {
            return item.uuid
          })
          $axios.post('/itilWorkflowCenter/approvePass', uuid, {
            data: params,
            logTemplate: `通过|工单(#sercode#)`
          }).then((res) => {
      ...
      diolagRequse(params, arr) {
        this.$refs.closeData.$refs.closeform.validate((valid) => {
          if (valid) {
            ...
            $axios.post('/itilWorkflowCenter/approveRefuse', params, {
              data: arr,
              logTemplate: `拒绝|工单(#sercode#)`
            }).then((res) => {
              if (res.data.state === true) {
                this.$message({
                  type: 'success',
      ...
      closeOK() {
        if (this.refuseuuid === '') {
          ...
          this.diolagRequse(params, this.selections)
        } else {
          this.diolagRequse({[this.refuseuuid]: this.$refs.closeData.closeform.reason}, this.refuseData)
        }
      },
