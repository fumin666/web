      ...
      <s-row>
        <s-form
          ...
          <s-form-item label="脚本库" prop="scriptLibUuid">
            <input-tree v-model="scriptForm.scriptLibUuid" :data="scriptLibraryList" :props="myProps" @select-change="changeScriptLibUuid" style="width: 528px;" name="uuid"></input-tree>
          </s-form-item>
          <s-form-item label="脚本名称" prop="scriptUuid">
            <s-select v-model="scriptForm.scriptUuid" style="width: 528px;">
              <s-option
                style="width: 660px;"
          ...
          <s-form-item label="超时时间" prop="timeOut">
            <s-input v-model="scriptForm.timeOut" style="width: 528px;" placeholder="秒，请输入正整数"></s-input>
          </s-form-item>
          <s-row>
            <s-col :span="20">
            ...
            <s-col :span="4" class="executeScriptBtn">
...
<script type="text/babel">
  ...
  export default{
    ...
    methods: {
      // 获取所有脚本库下拉选项
      getAllScriptLibrary() {
        $axios.get('/scriptConsole/scriptLibTreeNode').then(({data}) => {
          if (data && data instanceof Array) {
            this.scriptLibraryList = data;
          }
        })
      },
      // 脚本库选择变化 -> 获取脚本库下的脚本
      changeScriptLibUuid() {
        $axios.get(`/scriptConsole/getScriptByLibUuid/${this.scriptForm.scriptLibUuid}`).then(({data}) => {
          if (data && data instanceof Array) {
            this.scriptList = data;
            if (data.length > 0) {
              this.scriptForm.scriptUuid = data[0].uuid;
            } else {
      ...
      executeScript() {
        this.$refs.scriptForm.validate(valid => {
          if (valid) {
            ...
            let arr = this.scriptList.find(item => {
              return item.uuid === saveData.scriptUuid
            })
            $axios.post('/immediateExcuteTask/startImmediateExcuteScriptTask', saveData, {
              data: arr,
              logTemplate: '执行脚本|智能运维中心>脚本(#scriptName#)'
            }).then(({data}) => {
        ...
        $axios.post('/immediateExcuteTask/getExcuteScriptTaskResultList', param)
          .then(({data}) => {
            ...
            this.hostList.forEach(item => {
              data.forEach(i => {
                if (item.itcompUuid === i.assetUuid) {
