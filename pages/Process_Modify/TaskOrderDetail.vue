...
<script type="text/babel">
  ...
  import finishTaskOrder from './DetailDialog/FinishTaskOrder.vue';
    ...
    methods: {
      ...
      judgeDisabled(data) {
        ...
        if (data.changeStatus === 6) {
          ...
          // 当前用户 === 流程提交人
          if (userUuid === data.createUser) {
            // 工单为草稿状态 (taskStatus === 1)
            if (data.taskStatus === 1) {
              this.assignDisabled = this.receiveDisabled = this.finishDisabled = this.closeDisabled = this.cancelDisabled = true
            } else {
              // 其他状态
              this.allDisabled = true
            }
      ...
      getAssignGroups(companyUuid) {
        return new Promise((resolve, reject) => {
          $axios.get(`/itilChangeOrder/getAllChangeTaskAssignGroup/${companyUuid}`).then(({data}) => {
      ...
      getAssignUsers(groupUuid) {
        $axios.get(`/itilChangeOrder/getAllChangeChangeTaskAssignUserByGroup/${groupUuid}`).then(({data}) => {
          if (data && data instanceof Object) {
       ...
       */
      preGetSelectsList() {
        $axios.get(`/itilChangeOrder/getChangeTaskDetail/${this.taskOrderUuid}`).then(({data}) => {
          if (data && data instanceof Object) {
