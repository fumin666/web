          ...
          <s-form-item label="同步模式">
            <s-col span="10" >
              <s-select v-model="stepForm0.taskModel">
                <s-option label="LDAP组织结构" :value="2"></s-option>
                <s-option label="LDAP安全组" :value="3"></s-option>
              </s-select>
            </s-col>
          </s-form-item>
        </s-form>
      ...
      <div class="stepContent" key="2" v-else-if="currentStep == 2">
        <s-scrollbar wrap-class="scrollheight">
          <s-form ref="stepForm2" :model="stepForm2" :rules="stepRules2" label-width="120px">
            ...
            <s-form-item label="组织机构属性">LDAP属性</s-form-item>
            ...
            <s-form-item label="人员属性">LDAP属性</s-form-item>
...
<script>
  ...
  export default {
    data() {
      ...
      return {
        ...
        stepRules2: {
          ...
          defUserRole: [{type: 'string', required: true, message: '不能为空', trigger: 'change'}],
        ...
        form.validate((valid) => {
          ...
          if (this.currentStep > 2) {
            ...
            $axios.post(postUrl, sendData, {
              ...
              logTemplate: `${logTemplateTxt}|人员管理>LDAP导入(#taskName#)`
            }).then(res => {
    ...
    filters: {
      taskModel(val) {
        if (val === 2) return 'LDAP组织结构同步';
        if (val === 3) return 'LDAP安全组';
        return '';
      }
    },
    created() {
