    ...
    <s-row>
      <s-col :span="6">
        <div class="scriptItem">
          ...
          <div style="width:170px;margin:0 auto;word-wrap:break-word;">{{historyDetail.scriptName}}</div>
        </div>
      </s-col>
      <s-col :span="18">
        <s-row>
          <s-col :span="4" class="grid-content">执行人：</s-col>
          <s-col :span="8" class="grid-content">{{historyDetail.excuteUserName}}
          </s-col>
        ...
        <s-row>
          ...
          <s-col :span="8" class="grid-content">{{historyDetail.scriptLibName}}</s-col>
        ...
        <s-row>
          ...
          <s-col :span="8" class="grid-content">{{historyDetail.overtime}}秒</s-col>
        </s-row>
      </s-col>
    </s-row>
    <s-table :data='resultList'>
      <s-table-column label="主机名称" width="150px">
        <template slot-scope="scope">
          <div v-html="scope.row.name"></div>
          <div class="subContent" v-html="scope.row.ip"></div>
        </template>
      </s-table-column>
      <s-table-column prop="status" label="执行状态" width="120px">
        <template slot-scope="scope">
      ...
      <s-table-column label="执行结果">
        <template slot-scope="scope">
          ...
          <span v-if="!scope.row.result && scope.row.status"> -- </span>
        </template>
      </s-table-column>
    </s-table>
    </s-scrollbar>
