 ...
 * @Author: your name
 * @Date: 2019-08-02 14:09:54
 * @LastEditTime : 2019-12-27 15:24:09
 * @LastEditors  : mybells
 -->
<template>
  <div class="diy-oper">
    <s-row style="margin:10px 10px 0 10px">
      ...
      <s-col span="18" style="padding:15px;">
        ...
        <s-row>
          <s-col span="24" style="padding:0 15px;margin-top:20px;">
            <s-card class="table_card" style="margin-bottom:10px;">
                      ...
                      >
                        ...
                        >
                          <template slot-scope="scope">
                            <template v-if="item.colKey !== 'device_name'">{{scope.row[item.colKey]}}</template>
      ...
      <s-col span="6" style="padding:15px;">
        <s-card class="right-card">
          <s-scrollbar wrap-style="height: 310px;">
            <ul>
              <li class="team_li" @click="Order">
                <div class="team_box" style="background:#60ACFC">
                  <div
              ...
              <li class="team_li" @click="Synchronization">
                <div class="team_box" style="background:#5BC49F">
                  <div
              ...
              <li class="team_li" @click="assistance">
                <div class="team_box" style="background:#5BC49F">
                  <div
              ...
              <li class="team_li" @click="clientset">
                <div class="team_box" style="background:#FEB64D">
                  <div
        ...
        <s-card class="notice_card">
          <s-scrollbar wrap-style="height: 450px;">
            <div class="table_id">
              最新通知
              <div class="more-info" style="float:right;color:#199FED;cursor: pointer;" @click="Updatednot">更多</div>
            </div>
            <div style="margin:0 20px;" v-for="(item, index) in newEvents" :key="index">
              <div v-if='item.operationIncidentType===1'>
                <div style="height:20px;">
                  <div class="circular"></div>
                  <div class="notice_time">{{item.createTimeStr}}</div>
                  <div class="notice_maintenance" style="border:1px solid #FDA85C;color:#FDA85C;">运维审批</div>
                </div>
                <div class="notice_content">
                  <div class="notice_font">名称：{{item.scripttask.taskName}}</div>
                  <div class="notice_font">申请人：{{item.ScriptAudit.userRealName}}</div>
                  <div class="notice_font">执行时间：{{item.scripttask.beginTimeDate}}～{{item.scripttask.endTimeDate}}</div>
                  <div class="notice_font">执行周期：{{item.ScriptAudit.strPeriod}}</div>
                  <s-button style="margin-top:10px;" @click="EventGo(item)">审批</s-button>
                </div>
              </div>
              <div v-if='item.operationIncidentType===2'>
                <div style="height:20px;">
                  <div class="circular"></div>
                  <div class="notice_time">{{item.createTimeStr}}</div>
                  <div class="notice_maintenance" style="border:1px solid #168BF5;color:#168BF5;">运维会同</div>
                </div>
                <div class="notice_content">
                  <div class="notice_font">申请人：{{item.content.userRealName}}</div>
                  <div class="notice_font">IP: {{item.content.deviceIp}}</div>
                  <div class="notice_font">协议：{{item.content.protocolName}}</div>
                  <div class="notice_font">设备账号：{{item.content.accountName}}</div>
                  <s-button style="margin-top:10px;" @click="EventGo(item)">会同</s-button>
                </div>
              </div>
              <div v-if='item.operationIncidentType===3'>
                <div style="height:20px;">
                  <div class="circular"></div>
                  <div class="notice_time">{{item.createTimeStr}}</div>
                  <div class="notice_maintenance" style="border:1px solid #168BF5;color:#168BF5;">运维协助</div>
                </div>
                <div class="notice_content">
                  <div class="notice_font">申请人：{{item.content.userRealName}}</div>
                  <div class="notice_font">IP: {{item.content.deviceIp}}</div>
                  <div class="notice_font">协议：{{item.content.protocolName}}</div>
                  <div class="notice_font">设备账号：{{item.content.accountName}}</div>
                  <s-button style="margin-top:10px;" @click="EventGo(item)">接受</s-button>
                </div>
              </div>
              <div v-if='item.operationIncidentType===4'>
                <div style="height:20px;">
                  <div class="circular"></div>
                  <div class="notice_time">{{item.createTimeStr}}</div>
                  <div class="notice_maintenance" style="border:1px solid #168BF5;color:#168BF5;">运维复核</div>
                </div>
                <div class="notice_content">
                  <div class="notice_font">申请人：{{item.content.userRealName}}</div>
                  <div class="notice_font">IP: {{item.content.deviceIp}}</div>
                  <div class="notice_font">协议：{{item.content.protocolName}}</div>
                  <div class="notice_font">设备账号：{{item.content.accountName}}</div>
                  <s-button style="margin-top:10px;" @click="EventGo(item)">复核</s-button>
                </div>
              </div>
    ...
    <approved v-model="teamApply" v-if="teamApply"></approved>
...
<script>
  ...
  import approved from './Dialog/approved ';
  import Others from './Dialog/Others';
  import batchMe from './Dialog/batchMe';
  import approvedother from './Dialog/approvedother';
    ...
    methods: {
      ...
      getNewEventList() {
        $axios.get('operationCenter/getOperationPushLog').then(res => {
          let detail = res.data.slice(0, 4);
          this.newEvents = detail.map(val => {
            let inform = JSON.parse(JSON.parse(val.detailInfo).message)
            inform.createTimeStr = val.createTimeStr
            return inform
          });
          // this.newEvents.map(val => {
          //   if (val.operationIncidentType === 1) {
          //     this.newEventstime = this.formatDate(val.scripttask.createTime * 1000)
          //   }
          //   if (val.operationIncidentType === 4 || 3 || 2) {
          //     this.newEventstime = this.formatDate(val.content.createTime * 1000)
          //   }
          // })
        });
      },
