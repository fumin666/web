<template>
  <div>
    <s-dialog v-model="dialogFlag" title="邮件服务器设置" width="850px">
          ...
          <s-form-item label="邮件平台类型" prop="type">
            <s-col span="24">
              <s-select v-model="mailForm.type" placeholder="请选择所属机构" style="width:auto;">
                <s-option
                  v-for="(item,index) in emailPlatformList"
                  :key="index"
                  :label="item.name"
                  :value="item.name"
                ></s-option>
                  ...
                  <ul class="params-list">
                    <li v-for="(item,index) in emailParamsList" :key="index">
                      <div>
                        ...
                        <span>=</span>
                        <s-input
                          :type="emailParamsList[index].inputtype === 1? 'password' : 'text'"
                          v-model="emailParamsList[index].configValue"
                          style="width:auto;"
                        ></s-input>
                        <s-select
                          v-model="emailParamsList[index].inputtype"
                          placeholder="选择参数类型"
