<template>
  <s-dialog v-model="dialogFlag" title="微信服务器设置"  width="850px">
...
<script type="text/babel">
  ...
  export default {
    ...
    methods: {
      ...
      // 获取邮件短信服务器平台类型
      getServiePlatform() {
        $axios.get('/systemmanager/sysNotice/getSysNoticeByType/' + this.activeTab).then((res) => {
          if (res.data) {
            if (this.activeTab === 0) {
      ...
      // 根据获取邮件短信配置
      getServieConfig() {
        $axios.get('/systemmanager/sysConfig/getSysNoticedata/2' + this.activeTab).then((res) => {
          if (res.data) {
            if (this.activeTab === 0) {
              let arr1 = [];
      ...
      saveApprove() {
        ...
        if (editData.businessDatabaseInfoList === null || editData.businessDatabaseInfoList.length === 0) {
      ...
      saveStrategyForm() {
        ...
        _self.$refs['addStrategy'].$refs['Form'].validate((valid) => {
          if (valid) {
            $axios.post(`/configcenter/ipstrategy/addIpStrateg/${this.currentIPRuleData.nodeId}`, _self.$refs['addStrategy'].Form).then(({data}) => {
              console.log(data);
              if (data.result) {
                this.$message.success(data.errormsg);
              } else {
