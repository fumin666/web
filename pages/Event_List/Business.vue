  ...
  <section id="BusinessEvent">
      ...
      <s-form
        ...
        <div>
          ...
          <s-form-item label="服务器IP：" prop="server_ip">
            <s-input v-model="searchGroup.server_ip" placeholder="请输入" @keyup.enter.native="searchHandler"></s-input>
          </s-form-item>
    ...
    <page-infinite-table
      ...
      <s-table-column
        ...
        <template slot-scope="scope">
          <span v-if="option.colName === '事件标识'" :class="levelStatus[scope.row.alarm_level]" class="status-basic">{{scope.row.alarm_name}}</span>
          <span v-else-if="option.colName === '事件级别'" :class="levelStatus[scope.row.alarm_level]" class="status-basic" v-text="scope.row.alarm_name"></span>
    ...
    <s-dialog
      ...
      <template slot="footer" class="dialog-footer">
        <s-button @click="saveCreateEventOrder(4)">保存</s-button>
