<!--
 * @Description: 【报表中心】-【IPAM报表】-【IP地址状态汇总统计】-【子网视角】
 * @Author: 仝希博
 * @LastEditors: 高建鹏
 * @Date: 2019-04-22 11:19:58
 * @LastEditTime: 2019-06-11 10:01:31
 -->
...
export default {
  ...
  methods: {
    ...
    DownloadHandle(type) {
      ...
      this.$confirm(`确定将【IP地址状态汇总-子网视角】统计报表以${type}格式导出？`, '提示', {
        ...
        type: 'warning'
          ...
          $axios.post(`/ipamReport/unUsedOrAllIpOfSubnetExport`, params).then(res => {
              if (res.data === true) {
                Download(`/ipamReport/unUsedOrAllIpOfSubnetExportDownload/all/${type}`)
              } else {
                this.$message({
                  type: 'error',
                  message: `${type}导出失败`
