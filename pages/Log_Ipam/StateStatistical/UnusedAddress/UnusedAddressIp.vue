 ...
 * @LastEditTime: 2019-04-29 09:59:04
 * @LastEditTime: 2019-06-25 11:11:56
 -->
 -->
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        $axios.post(`/ipamReport/unUsedOrAllIpOfSelf`, parms).then((res) => {
          this.tableData = res.data || []
        })
      },
      DownloadHandle(type) {
        this.$confirm(`确定将【未使用的IP地址-IP地址视角】统计报表以${type}格式导出？`, '提示', { confirmButtonText: '确定', cancelButtListonText: '取消', type: 'warning' }).then(() => {
          this.form.exportType = type
          $axios.post(`/ipamReport/unUsedIpOfSelfExport`, this.form).then(res => {
              if (res.data === true) {
