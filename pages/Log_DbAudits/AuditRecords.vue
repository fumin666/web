...
export default {
  ...
  methods: {
    ...
    getList() {
      const form = this.formData()
      $axios.post('/dbauditReport/auditDB', form).then((res) => {
    ...
    downloadHandle(type) {
      ...
      Download(`/dbauditReport/auditDBExport/${form.startTime}/${form.endTime}/${form.timeType}/${type}`);
    }
  },
  mounted() {
    this.getList()
