...
*/
<template>
  <section class="content logSpecialAudit">
    ...
    <s-table-page
      ...
      <s-table-column label="通过次数" prop="approvePassCount"></s-table-column>
      <s-table-column label="拒绝次数" prop="approveRefuseCount"></s-table-column>
    </s-table-page>
    <!--点击选择机构-->
    <s-dialog
...
//        this.collectTask.taskPeriod = Number(this.collectTask.taskPeriod);
      ...
      showList() {
        let vm = this;
        $axios.post('/operationWorkStat/specialoperApproveCount', {departUuids: vm.institutionUuid, endTime: vm.endTimes, startTime: vm.beginTimes, specialTypeUuids: vm.specialTypeUuids}).then((res) => {
      ...
      DownloadHandle(type) {
        let vm = this;
        $axios.post('/operationWorkStat/makeSpecialApproveCountFile', {exportType: type, departUuids: vm.institutionUuid, endTime: vm.endTimes, startTime: vm.beginTimes, specialTypeUuids: vm.specialTypeUuids}).then((res) => {
          if (res.data) {
            Download(`/operationWorkStat/downSpecialApproveWorks/${type}`);
          }
        });
      },
      getOpsType() {
