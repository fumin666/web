<template>
  <section class="content">
    <s-form :inline="true">
      <select-time ref="selecttime">
        <template slot="serachForm">
          ...
          <s-form-item label="DCL操作指令：" label-width="110px">
    ...
    <s-table-page
      ...
      <s-table-column label="DCL操作指令" prop="cmd"></s-table-column>
    ...
    methods: {
      ...
      DownloadHandle(type) {
        ...
        $axios.post('/dbauditReport/operationExport', Object.assign({}, vm.params, vm.period, {exportType: type})).then((res) => {
          if (res.data === true) {
            Download(`/dbauditReport/operationExportDownload/dcl/${type}`);
          }
        });
      }
    },
