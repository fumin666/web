<template>
  <section class="content">
    <s-form :inline="true">
      <select-time ref="selecttime">
        <template slot="serachForm">
          ...
          <s-form-item label="DDL操作指令：" label-width="110px">
    ...
    <s-table-page :data="topLists">
      ...
      <s-table-column label="DDL操作指令" prop="cmd"></s-table-column>
    ...
    methods: {
      ...
      DownloadHandle(type) {
        ...
        $axios.post('/dbauditReport/operationExport', Object.assign({}, vm.params, vm.period, {exportType: type})).then((res) => {
          if (res.data === true) {
            Download(`/dbauditReport/operationExportDownload/ddl/${type}`);
          }
        });
      }
    },
