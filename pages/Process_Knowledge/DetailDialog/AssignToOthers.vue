<template>
  <s-scrollbar wrap-style="max-height:500px;">
    <s-form
      ...
      label-width="200px">
      <s-form-item label="知识库管理员组名称：" prop="groupUuid">
      ...
      <s-form-item label="知识库管理员名称：" prop="userUuid">
  ...
  export default{
    ...
    watch: {
      'assignToOthersForm.groupUuid'(newVal) {
        $axios.get(`/itilKnowledgeOrder/getAllKnowledgeaAdminUserByGroup/${newVal}`).then(({data}) => {
    ...
    methods: {
      saveAssignToOthers() {
        this.$refs.assignToOthersForm.validate((valid) => {
          if (valid) {
            $axios.get(`/itilKnowledgeOrder/knowledgeOrderAssignOther/${this.orderUuid}/${this.assignToOthersForm.groupUuid}/${this.assignToOthersForm.userUuid}`).then(({data}) => {
              if (data.state === true) {
                this.$message({
                  type: 'success',
                  message: '指派成功！'
    ...
    created() {
      // 获取变更管理员组
      $axios.get('/itilKnowledgeOrder/getAllKnowledgeAdminGroup/' + this.companyUuid).then(({data}) => {
        if (data && data instanceof Array) {
