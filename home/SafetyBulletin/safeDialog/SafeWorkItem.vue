  ...
  <div style="margin-right: 10px;margin-top: 10px;width: calc(33.3% - 12px);overflow: hidden;" class="DealWorkBox">
    <div class="dealWorkContainer">
      <s-row class="title" >
        ...
        <s-col span="6" style="height:157px" class="dispose process-border-left">
          <div class="btns" style="text-align: center;font-size:18px;bottom:8px"> <!--事件，任务按钮-->
            <div :max="99" :title="model.orderType === 1 && model.orderStatus === 1 ? '工单未启动,请在启动时间后操作' :'处理'" v-if='flowType === 1' style="margin-top:20px">
              <s-button class="btnCol" icon="order-track" :disabled='model.orderType === 1 && model.orderStatus === 1' @click="routerJump(model)">
              </s-button>
            </div>
            <div :title="model.orderType === 1 && model.orderStatus === 1 ? '工单未启动,请在启动时间后操作' :'委派'" :max="99" v-if='flowType === 1' style="margin-top:20px">
              <s-button icon="assigned" class="btnCol" :disabled='model.orderType === 1 && model.orderStatus === 1' @click="designate()">
              </s-button>
            </div>
            <div title="通过" :max="99" v-if='flowType === 2' style="margin-top:20px">
              <s-button icon="finish" class="btnCol" @click="appove(1)">
              </s-button>
            </div>
            <div title="拒绝" :max="99" v-if='flowType === 2' style="margin-top:20px">
              <s-button icon="cancel" class="btnCol" @click="appove(0)">
              </s-button>
            </div>
            <div :title="model.orderStatus === 1 ? '工单未启动,请在启动时间后操作' :'参加'" :max="99" v-if='flowType === 3 && model.isMeeting === 1' style="margin-top:20px">
              <s-button icon="finish" :disabled='model.orderStatus === 1' class="btnCol" @click="receipt(2)">
              </s-button>
            </div>
            <div :title="model.orderStatus === 1 ? '工单未启动,请在启动时间后操作' :'拒绝'" :max="99" v-if='flowType === 3 && model.isMeeting === 1' style="margin-top:20px;">
              <s-button icon="cancel" :disabled='model.orderStatus === 1' class="btnCol" @click="receipt(3)">
              </s-button>
            </div>
            <div :title="model.orderStatus === 1 ? '工单未启动,请在启动时间后操作' :'回执'" :max="99" v-if='flowType === 3 && model.isMeeting !== 1' style="margin-top:55px;">
              <s-button icon="email" :disabled='model.orderStatus === 1' class="btnCol" @click="receipt(1)">
              </s-button>
            </div>
          </div>
        </s-col>
    ...
    <Appoint-task-dialog v-model="AppointTaskDialog" v-if="AppointTaskDialog" :taskName='taskName' :orderUuid='orderUuid' @receive='receive'></Appoint-task-dialog>
  ...
  export default {
    ...
    methods: {
      ...
      handleConcern() {
        ...
        let str = this.model.isConcern === 0 ? `关注|通报(${this.model.safeId})` : `取消关注|通报(${this.model.safeId})`
        ...
        this.$refs['appoveForm'].validate((valid) => {
          if (valid) {
            ...
            $axios.post('/safetyPlatform/safeOrder/approvalOrder', parms, {
              logTemplate: `处理|通报(${this.model.safeId})`
            }).then(res => {
                if (res.data.state) {
      ...
      routerJump(model) {
        if (model.orderType === 1) {
          this.$router.push(`/SafetyBulletin/safetyBulletin_details/${model.uuid}`)
        } else {
          this.$router.push(`/SafetyBulletin/safetyBulletin_dispose/${model.uuid}`);
        }
      },
      getOrderType(num) {
        switch (num) {
